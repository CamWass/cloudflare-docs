...
{ props.magicWord === "hardware" && (
  <>
		<p>You need to purchase <a href="https://www.cloudflare.com/magic-wan/">Cloudflare WAN</a> before you can purchase and use {props.productName}. {props.productName} can function as your primary edge device for your network, or be deployed in-line with existing network gear.</p>
...
{ props.magicWord === "virtual" && (
  <>
		<Markdown
		text={`
		Before you can install ${props.productNameVirtual}, you need an Enterprise account with Cloudflare WAN. Additionally, you need to have a VMware or Proxmox host with sufficient compute, memory, and storage to run the virtual machine with ${props.productNameVirtual}. This includes:
...
There are a couple of decisions you need to make when installing your { props.magicWord === "virtual" ? props.productNameVirtual : props.productName }. Review the following topics for more information.
...
{ props.magicWord === "hardware" && (
  <>
    <p>You can use {props.productName} in both DHCP networks and networks that require a static IP configuration. At first boot, however, {props.productName} needs to reach out to Cloudflare to download your settings and go through the activation process. If any of the networks plugged into your {props.productName} device are DHCP enabled, do not use a VLAN, and have an Internet connection, that process is handled automatically. However, if all of the networks require more information to utilize, (such as a network with static IPs, or tagged VLAN networks) your {props.productName} might need some more information to proceed.</p>
		...
		2. Follow the [setup flow](#set-up-cloudflare-dashboard) and activate your ${props.productName} device.
...
{ props.magicWord === "virtual" && (
  <>
    ...
    1. Connect the machine with your ${props.productNameVirtual} VM to a DHCP port with access to the Internet.
		2. Follow the [setup flow](#set-up-cloudflare-dashboard) and activate your ${props.productNameVirtual} device.
		3. Refer to [WAN with a static IP address](#wan-with-a-static-ip-address).
		`}
		inline={false}
		/>
...
{ props.magicWord === "virtual" && (
  <>
		...
		<p>Select the appropriate tab to configure {props.productNameVirtual} on VMWare ESXi or Proxmox Virtual Environment.</p>
...
{/* CF1 Cloudflare WAN section */}
{ props.dashName === "CF1 Cloudflare WAN" && (
	<>
		<ol>
			<li>Log in to <a href="https://dash.cloudflare.com/">Cloudflare dashboard</a>, and go to <strong>Networks</strong>.</li>
...
1. In **WAN configuration**, select **Create**. You can create one or more [wide area networks (WANs)](https://www.cloudflare.com/learning/network-layer/what-is-a-wan/). Configuring multiple WANs will create multiple <GlossaryTooltip term="IPsec tunnel">IPsec</GlossaryTooltip> tunnels (one IPsec tunnel per WAN port). This allows { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } to load balance traffic over WANs of equal priority. It also allows { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } to failover between circuits according to their <GlossaryTooltip term="tunnel health-check" link={props.tunnelHealthChecksURL}>health</GlossaryTooltip>. Refer to <a href={props.wanSettingsURL}>WAN settings</a> for more details.
		:::note
		This is not the same as a high availability (HA) configuration. HA configurations need two { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } devices to work. For details, refer to [About high availability configurations](#about-high-availability-configurations).
		:::
...
5. In **Priority**, choose the priority for your WAN. Lower numbers have higher priority. For details on how Cloudflare calculates priorities, refer to <a href={props.trafficSteeringURL}>Traffic steering</a>.
6. In **Health check rate** configure the health check frequency for your site. Options are `low`, `mid`, and `high`. For details, refer to <a href={props.healthCheckFrequencyURL}>Update tunnel health checks frequency</a>.
7. **Addressing**: Select **DHCP**. This is needed the first time you set up your { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } to successfully download all settings to the machine and activate it. If you need a static IP address in your network environment:
	1. Continue the set up flow to activate your { props.magicWord === "virtual" ? props.productNameVirtual : props.productName }.
...
7. (Optional) If your LAN contains additional subnets behind a layer 3 router, select **Add routed subnet** under **Routed subnets** to add them:
   ...
   - **Static NAT prefix**: Optional setting. If you want to enable NAT for a routed subnet, supply an "external" prefix for the overlay-facing side of the NAT to use. It must be the same size as **Prefix**. <br /> For details, refer to <a href={props.routedSubnetsURL}>Routed subnets</a>.
8. Select **Save**.
...
After setting up your LANs, you can configure your { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } to enable communication between them without traffic leaving your premises. For details, refer to <a href={props.networkSegmentationURL}>Network segmentation</a>.
...
{ props.magicWord === "hardware" && (
  <>
    ...
    class a,c orange`}
		...
		accDescr: ${props.productName} connects to the router in the site, and only some of the LANs connect to Appliance.
    ...
    class a,i orange`}
		...
		<p><em>{props.productName} connects to the router in the site, and only some of the LANs connect to Appliance.</em></p>

		<p>Refer to <a href="/reference-architecture/diagrams/sase/cloudflare-one-appliance-deployment/">{props.productName} deployment options</a> for a high-level explanation of the deployment options that make sense to most environments, as well as a few advanced use cases.</p>
...
{ props.magicWord === "hardware" && (
  <>
		...
		<p> If you set up your {props.productName} with a static IP through the bootstrap method, you do not need a DHCP port. For details, refer to <a href="#decide-on-dhcp-vs-static-ip-connections"> DHCP vs static IP connections</a>.</p>
...
{/* Instructions for CF1 Cloudflare WAN */}
{ props.dashName === "CF1 Cloudflare WAN" ? (
	<>
		<Markdown
	text={`
		1. Log in to <a href="https://dash.cloudflare.com/">Cloudflare dashboard</a>, and go to **Networks**.
		...
		5. The **Interrupt window** is the time period when the ${ props.magicWord === "virtual" ? props.productNameVirtual : props.productName } software can update, which may result in interruption to existing connections. Choose a time period to minimize disruption to your sites. For details on defining when the ${ props.magicWord === "virtual" ? props.productNameVirtual : props.productName } can update its systems, refer to <a href="${ props.interruptWindowURL }">Interrupt window</a>.
		6. Select **Update**.
	`}
	inline={false}
	/>
	...
	<>
		{/* Instructions for Cloudflare WAN */}
		<Markdown
	text={`
...
{ props.magicWord === "virtual" && (
  <>
		<AnchorHeading title="Boot your Virtual Appliance" depth={2} />

		<AnchorHeading title="Default password to access Virtual Appliance" depth={3} />
...
{ props.magicWord === "hardware" && (
  <>
    <p>After activating your device, you can use it in a network configuration with the WAN interface set to a static IP address — that is, an Internet configuration that is not automatically set by DHCP. To use your {props.productName} on a network configuration with a static IP, follow these steps:</p>
...
{ props.magicWord === "virtual" && (
  <>
		<p>After activating your device, you can use it in a network configuration with the WAN interface set to a static IP address - that is, an Internet configuration that is not automatically set by DHCP. To use your {props.productNameVirtual} on a network configuration with a static IP, follow these steps:</p>
...
{ props.magicWord === "hardware" && (
  <>
		...
		<p>The following is a detailed description of how to use the serial port to configure your {props.productName} locally.</p>
		...
		<p>To access the {props.productName} device environment you need a serial terminal client. Follow these instructions to install one, based on your operating system.</p>
...
5. From the list, choose your first { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } > **Add Appliance**.
6. Back on the previous screen, select **Add secondary appliance**.
7. From the list, choose your second { props.magicWord === "virtual" ? props.productNameVirtual : props.productName } > **Add Appliance**.
...
15. {props.magicWord === "hardware" && (<>Follow the instructions in <a href="#set-up-your-cloudflare-one-appliance">Set up your Cloudflare One Appliance</a> and <a href="#activate-appliance">Activate appliance</a> to finish setting up your Appliances.</>)} {props.magicWord === "virtual" && (<>Follow the instructions in <a href="#activate-appliance">Activate appliance</a> to finish setting up your Appliances.</>)}
...
{ props.dashName === "CF1 Cloudflare WAN" ? (
	<>
		<Markdown
	text={`
		1. Log in to <a href="https://dash.cloudflare.com/">Cloudflare dashboard</a>, and go to **Connectors**.
		2. In **Cloudflare WAN** you can inspect the IPsec tunnels created by your ${props.magicWord === "virtual" ? props.productNameVirtual : props.productName }.
		3. In **Routes** you can inspect the static routes created by your ${props.magicWord === "virtual" ? props.productNameVirtual : props.productName }.
   ...
   <DashButton url="/?to=/:account/magic-networks/connections" />
		...
		<Markdown
	text={`
	2. The **IPsec/GRE tunnels** tab shows a list of all the IPsec tunnels created by your ${props.magicWord === "virtual" ? props.productNameVirtual : props.productName }.
	...
	<DashButton url="/?to=/:account/magic-networks/routes" />
		...
		<Markdown
	text={`
	4. Here you can inspect the static routes created by your ${props.magicWord === "virtual" ? props.productNameVirtual : props.productName }.
	`}
	inline={false}
	/>
	</>
